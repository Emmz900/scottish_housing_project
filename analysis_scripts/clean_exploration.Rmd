...
average_community_belonging <- community_belonging %>% 
  filter(measurement == "Percent") %>% 
  summarise(avg_rating = mean(belonging_score)) %>% 
  pull()
...
null_dist <- community_belonging %>% 
  ...
  specify(belonging_score ~ type_of_tenure) %>% 
...
null_dist <- community_belonging %>% 
  ...
  #select(belonging_score) %>% 
  specify(response = belonging_score) %>% 
...
```{r}
null_dist <- community_belonging %>% 
  select(belonging_score, type_of_tenure) %>% 
  mutate(owned_outright = (type_of_tenure == "Owned Outright")) %>% 
  select(-type_of_tenure) %>% 
  specify(belonging_score ~ owned_outright) %>% 
  hypothesise(null = "independence") %>% 
  generate(reps = 500, type = "permute") %>% 
...
null_dist <- community_belonging %>% 
  ...
  select(-type_of_tenure) %>% 
  specify(belonging_score ~ owned_mortgage) %>% 
  hypothesise(null = "independence") %>% 
  generate(reps = 500, type = "permute") %>% 
  calculate(stat = "diff in means", order = c(TRUE, FALSE))
...
null_dist <- community_belonging %>% 
  ...
  select(-type_of_tenure) %>% 
  specify(belonging_score ~ rented) %>% 
  hypothesise(null = "independence") %>% 
  generate(reps = 500, type = "permute") %>% 
  calculate(stat = "diff in means", order = c(TRUE, FALSE))
